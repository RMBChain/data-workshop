...
| 日期 | 说明 |
|------|------|
| 2026-04-22 | 初版：对齐需求/UI，并标注与当前 `backend` 已实现端点差异及推理上传待收敛项 |
| 2026-04-22 | 总则 §1.6：MVP 持久化暂用 SQLite，区分库内元数据与文件系统大文件 |
| 2026-04-22 | 总则 §1.7：ms-swift 通过 CLI 子进程调用（swift / python -m swift），环境与日志约定 |
| 2026-04-22 | §5：训练日志与 UI 滚动约定（轮询 + 跟随尾部/暂停跟随）；§3.3 UI 补充「训练日志区」 |
