...
def list_merge_page_training_rows(workspace: Path) -> list[dict[str, Any]]:
    ...
    conn = get_connection(root)
    rows = conn.execute(
        "SELECT id, request_json, status, error_message FROM dws_trains "
...
def list_workspace_models(workspace: Path) -> list[dict[str, Any]]:
    ...
    root = workspace.resolve()
    out: list[dict[str, Any]] = []
    scan_roots = [root / "output", root / "merged"]
    scan_dirs = [d for d in scan_roots if d.is_dir()]
    if not scan_dirs:
        return []
