...
def train_with_swift(
    ...
    **kwargs,
    ...
    kwargs.setdefault("image_max_token_num", 256)
    ...
    env["IMAGE_MAX_TOKEN_NUM"] = str(kwargs.get("image_max_token_num", 256))
    env["VIDEO_MAX_TOKEN_NUM"] = str(kwargs.get("video_max_token_num", 64))
    ...
    argv: list[str] = [
        ...
        output_dir,
        "--lora_rank",
        str(kwargs.get("lora_rank", 4)),
        "--lora_alpha",
        str(kwargs.get("lora_alpha", 8)),
        "--target_modules",
        ...
        str(kwargs.get("gradient_accumulation_steps", 8)),
        "--learning_rate",
        ...
        "false",
        "--max_length",
        str(kwargs.get("max_length", 512)),
        "--logging_steps",
        str(kwargs.get("logging_steps", 5)),
        "--save_steps",
        str(kwargs.get("save_steps", 200)),
        "--eval_steps",
        str(kwargs.get("eval_steps", 200)),
        "--save_total_limit",
        str(kwargs.get("save_total_limit", 2)),
    ...
    print(f"  max_length: {kwargs.get('max_length', 512)}")
...
def main():
    ...
    parser.add_argument("--lora_rank", type=int, default=4, help="LoRA rank（默认宜小以省内存）")
    ...
    parser.add_argument("--max_length", type=int, default=512, help="最大序列长度（过大易 OOM）")
    parser.add_argument("--logging_steps", type=int, default=20, help="日志步频")
    parser.add_argument("--save_steps", type=int, default=500, help="保存步频")
    parser.add_argument("--eval_steps", type=int, default=500, help="评估步频")
    ...
    parser.add_argument("--image_max_token_num", type=int, default=256, help="图片最大 token 数（VL 主内存占用之一）")
    parser.add_argument("--video_max_token_num", type=int, default=64, help="视频最大 token 数")
    parser.add_argument(
        "--swift_callbacks",
        nargs="*",
        default=None,
